/-
  ...
  Nontrivial cycles contradict Baker's drift bound.

  **Proof**: Assume a nontrivial realizable cycle with fixed profile P.
  1. Realizability gives fixed point n₀ = W/D > 0
  2. One cycle scales by 3^m/2^S = 2^{-ε} where ε = S - m·log₂(3)
  3. L cycles scale by (3^m/2^S)^L = 2^{-Lε}
  4. Baker's theorem: |ε| ≥ δ/m > 0 for nontrivial P
  5. Choose L = ⌈m/δ⌉ so |Lε| ≥ 1
  6. Then n₀ · 2^{-Lε} ≠ n₀, but fixed profile requires return to n₀
  7. Contradiction.
-/
...
/-- A cycle is realizable, nontrivial, and returns to its starting value after L applications of profile P. -/
...
/-- Two-profile bridge: integer-profile exact return implies real-profile exact
return at drift exponent `0`. -/
...
/-- No nontrivial realizable cycle can have a fixed profile.
    Baker's drift bound forces |Lε| ≥ 1 for some L, which prevents orbit closure. -/
...
/-! ## Corollary: Connection to full Collatz conjecture -/
