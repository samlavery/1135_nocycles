/-
  ...
  Proves: n₀ · (2^S − 3^m) = waveSum for periodic odd orbits.

  Ported from finallean/Collatz/Basic.lean (`cycle_equation`).
-/
import Collatz.Defs
...
/-- Cycle equation: n₀ · (2^S − 3^m) = c_m for periodic orbits of period m. -/
...
/-- Exponential growth inequality: 2^{S_m} > 3^m for periodic orbits. -/
