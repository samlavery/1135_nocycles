/-
  NoCycle.lean — No nontrivial Collatz cycle exists
  ==================================================
  ...
  **Cycle structure**: A cycle of length m has profile ν = (ν₀, ..., ν_{m-1}),
  ...
  Realizability requires W = n₀·D for some odd positive integer n₀.

  **Proof**: Lattice intersection emptiness.
  - Realizability forces n₀ into a coset: n₀ ∈ L₂(σ) = {n : 2^S | (W + n·3^m)}
  - For nontrivial profiles, valuation bound gives v₂(W + n₀·3^m) < S, so n₀ ∉ L₂(σ)
  - Contradiction: no integer start value can realize a nontrivial pattern

  **Trivial profile**: ν = (2,...,2) gives W = D, n₀ = 1, satisfying the lattice condition.
  This is why 1→4→2→1 exists and nothing else.

  **Axioms**: Baker/LMN bounds, Hardy-Ramanujan/Simons-de Weger cycle length bounds.
-/
import Collatz.Defs
...
/-- For d ≥ 2, there exists a prime p dividing 4^d - 3^d. -/
...
/-! ═══════════════════════════════════════════════
    ...
    **Proof via DriftContradiction (primary):**
    Uses Baker's theorem + drift accumulation.
    - ε = S - m·log₂(3) ≠ 0 (Baker transcendence bound)
    - Each loop adds drift ε: after L loops, orbit at n₀ · 2^{-Lε}
    - When |Lε| ≥ 1: n₀ · 2^{-Lε} ≠ n₀ → orbit changed to different number ✓
    - No constant cycle profile can exist
    - Machinery: Transcendental number theory (irrational approximation)

    **Alternative proof available (LatticeProof):**
    Uses cyclotomic rigidity + 2-adic valuation.
    - D | W → all νⱼ equal (Steiner/Eliahou)
    - Contradiction with nontriviality ✓
    - Machinery: Algebraic number theory (cyclotomic rings, 4-adic cascade)

    **Geometric picture:**
    Both prove the same lattice intersection emptiness:
    - ℤ lattice: Integer Collatz values
    - ℝ dynamics: Powers of log₂(3) (transcendental)
    - Unique intersection: Trivial cycle (m=1, D=1)
    - For m≥2: Lattices never align (Baker drift + cyclotomic rigidity)

    DriftContradiction shows the "quantization error" mechanism: forcing S, m ∈ ℤ
    when log₂(3) is irrational creates drift. Like floating-point errors
    accumulating over iterations, the orbit drifts away and breaks the cycle. -/
...
theorem no_nontrivial_cycles_sublattice {m : ℕ} (hm : m ≥ 2)
  ...
  have hn₀_pos : n₀ > 0 := by
    ...
    have : (P.waveSum : ℤ) ≤ 0 := by
      rw [hW_eq]
      exact mul_nonpos_of_nonpos_of_nonneg h (le_of_lt hD_pos)
    linarith
...
/-- **No nontrivial cycles via DriftContradiction.**
    Uses Baker's theorem and drift accumulation. Each loop adds drift ε,
    so after L loops the orbit is at n₀ · 2^{-Lε} ≠ n₀ (different number!).
    This proves no fixed-profile cycle can exist. -/
...
This is the bridge from the full Collatz map (collatzIter) to the Syracuse
formulation (collatzOddIter). Given a collatzIter cycle with n > 4, we:
1. Extract a Syracuse cycle (odd subsequence)
2. Build a CycleProfile from the Syracuse cycle
3. Show it's realizable and nontrivial
4. Pass to no_nontrivial_cycles (uses DriftContradiction + LatticeProof)
-/
