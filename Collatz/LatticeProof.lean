/-
  LatticeProof.lean — Lattice non-membership
  ===========================================

  Proves: For nontrivial CycleProfile with D > 0, D ∤ W.

  Strategy: Decompose E = A + B where v₂(A) = K, v₂(B) = ν₀.
  Forced alignment gives K = ν₀. Carry bound shows v₂(E) < S,
  contradicting membership v₂(E) = S.
-/
import Collatz.Defs
...
/-- For the trivial profile ν = (2,...,2), the orbit equation is
    ...
    This follows from W = D = 2^S - 3^m (proved in NoCycle.trivial_profile_residue_zero).

    Here we give a direct proof from the definitions. -/
